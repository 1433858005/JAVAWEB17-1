...
//添加学生信息
    ...
    public int insert(Student stu) throws SQLException {
        ...
        String sql = "INSERT INTO student.student VALUES (' "+stu.getSid()+"','"+stu.getName()+"','"+stu.getAge()+"','"+birthday+"')";
...
//修改学生信息
    ...
    public int update(Student stu) throws SQLException {
        ...
        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
        String birthday = sdf.format(d);
        String sql = "UPDATE student.student SET sid='"+stu.getSid()+"',name='"+stu.getName()+"',age='"+stu.getAge()+"',birthday='"+birthday+"' WHERE sid='"+stu.getSid()+"'";
        result=stat.executeUpdate(sql);
        JDBCUtils.close(con,stat);
        return result;
    }
